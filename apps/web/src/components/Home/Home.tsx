...
export function Home() {
  ...
  useEffect(() => {
    async function handleSearchButton(e: SubmitEvent) {
      ...
      // `/profile?gameName=${gameName_tag[0]}&tag=${gameName_tag[1]}&region=${region}`
      navigate({to: '/profile', search: {gameName: gameName_tag[0], tag: parseInt(gameName_tag[1]), region: region}});
    }
  ...
  return (
    <div id="home-wrapper">
      ...
      <form id="profile-search-form" autoComplete="off" ref={formRef}>
        <select id="region-choice" ref={regionSelectRef} className="region-select" name="region">
