...
export function Comments({puuid}: CommentsProps) {
    ...
    useEffect(() => {
        ...
        return () => {
            button.removeEventListener("click", handleCommentSubmission);
        }
    }, [])
    useEffect(() => {
    ...
    return (
        <Fragment>
            <form id="comment-form" action="">
                <textarea maxLength={140} id="comment-textarea" placeholder='Write a comment (140 character limit)' name="comment"></textarea>
                ...
                {comments != null ? comments.map(comment => {
                    return (
                        <div className='comment'>
                            <p className='comment-content'>{comment != null ? comment.content : <Loading/>}</p>
                            <h5>{comment != null ? comment.username : <Loading/>} 21/12/2014 08:07:54 </h5>
                        </div>
                    )
                }) : <Loading/>}
            </div>
